...
Scripts are numbered by stage below; run them in this order, as later scripts read `.rds` objects written by earlier ones.
  ...
  **Requirements:** R ≥ 4.3 with Bioconductor (`dada2`, `phyloseq`, `DECIPHER`,
  ...
  and eMPRess. Taxonomy assignment requires the GTDB SSU reference (`bac120_arc53_ssu_r214`).

  Download the data from Zenodo (10.5281/zenodo.20126521) into `data/`.

  ### 1. Preprocessing 16S and diet data (`preprocessing/metabarcoding_data/`)
  ...
  6. `vennSampleSize.rmd` — final overlapping sample 
  ...
  4. `iqtree_mt.sh` — phylogenetic inference
  5. `renameVCF.sh` — rename sample names in vcf file
  6. `bcftools_rohs.sh` — infer runs of homozygosity (ROH)
  7. `process_rohs.R` — calculate proportion of autosomal genome in ROHs and filter data
  8. `plink_PCA_noLD.sh` — Principal component analysis WGS data
  9. `draw_host_mt_nuclear_tree.rmd` — draw host tree
  10. `compute_Fst.sh` — compute popution-wise Fst 


  ### 3. Analyses and figures (`analysis/`)
  - `pca_WGS.rmd`, `nmds_16S.rmd`, `nmds_diet.rmd` — ordination and PERMANOVA

  #### Island biogeography but not colonisation history explains microbiome diversity
  - `SharedTaxa.rmd`, `SharedTaxa_UniFrac.rmd` — shared taxa analyses
  - `Figure3_plots.Rmd`, `Suppl_triangle.Rmd` — manuscript figures 

  #### Host genetic effects are stronger for abundant than rare microbial taxa
  - `MRM_mantel_tests_mtHostTree.rmd` — Mantel and MRM tests
  - `test_fst_froh.rmd`, `test_fst_heteroz_clean.rmd` — host genetic effects
  - `Figure4_plots.Rmd` — manuscript figure 

  #### Evidence of co-phylogeny but not co-diversification
  - `test_neutrality_allTaxa.rmd` — neutral model fitting
  ...
  - `Figure5_plots.rmd` — manuscript figure 
...
│   └── wgs_data/
│       ├── 01_run_genotyping_cesga.sh
│       ├── 02_filterVCF_chaffinch.sh
│       ├── 03_generate_consensus_depthFreq_mask.slurm
│       ├── 04_iqtree_mt.sh
│       ├── 05_renameVCF.sh
│       ├── 06_bcftools_rohs.sh
│       ├── 07_process_rohs.R
│       ├── 08_plink_PCA_noLD.sh
│       ├── 09_draw_host_mt_nuclear_tree.rmd
│       └── 10_compute_Fst.sh
├── 02_analysis/
│   ├── 01_pca_WGS.rmd
│   ├── 02_nmds_16S.rmd
│   ├── 03_nmds_diet.rmd
│   ├── 04_Figure3_tests_plots.Rmd
│   ├── 05_Suppl_triangle.Rmd
│   ├── 06_SharedTaxa.rmd
│   ├── 07_SharedTaxa_UniFrac.rmd
│   ├── 08_MRM_mantel_tests_mtHostTree.rmd
│   ├── 08_test_fst_froh.rmd
│   ├── 09_Figure4_plots.rmd
│   ├── 10_list_topFamilyFix_overallData.rmd
│   ├── 11_PACo_global_tests.rmd
│   ├── 12_PACo_core_families.rmd
│   ├── 13_prepare_eMPRess_input.rmd
│   ├── 14_test_neutrality_allTaxa.rmd
│   └── 15_Figure5_plots.rmd
